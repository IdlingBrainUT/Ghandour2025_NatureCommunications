...
function [result_sum_dur_range, data_for_hist1, data_for_hist2, i] = ...
    func_bin_ocurrence_hist_single(ca_filt_data, cut_frame_num, bin_frame_num, Cell_type_A_ID, cal_range_start, cal_range_end)

% ca_filt_data -> input data
% cut_frame_num = 0; % 0 = no cut, 1 = 50ms-frame-firing cut
% bin_frame_num = 10; % 500ms binning
% cal_range_start = 2330; % input range in second scale
% cal_range_end   = 2390; % input range in second scale
%     Cell_type_A_ID = [6,10,13,22,30,37,41,44,45,56,59]; % input PRE-Ai5 cell ID
%     Cell_type_B_ID = [3,8,35]; % input SQARE-Ai5 cell ID
%     Cell_type_C_ID = [9,12,34,43,48,63,65]; % input Shock-Ai5 cell ID
...
%                Cells_A_B{i,ii,iii} =1;
               Cells_A_logical(i,iii) =1; 
            else
%                Cells_A_B{i,ii,iii} =0;
                Cells_A_logical(i,iii) =0;
            end
    end
% end
end
...
% Cells_A_B_res_A = sum(Cells_A_logical,2); 
...
% 
% Cells_A_B_res_B = sum(Cells_A_logical,1); 
...
Cells_A_res_occurance_raster = [Cells_A_logical'];
Cells_A_res_occurance_sum = sum(Cells_A_res_occurance_raster,2);
Cells_A_res_occurance_vec = Cells_A_res_occurance_sum > 0;
...
% figure; 
subplot(222)
imagesc( (1:size(Cells_A_res_occurance_raster',2))*bin_frame_num./20 , (1:size(Cells_A_res_occurance_raster',1)) ,Cells_A_res_occurance_raster')
...
data_range_vec = Cells_A_res_occurance_vec(cal_range_for_bin_data,:);
...
%% figure Occurance hist
    ...
    data_range_sum = Cells_A_res_occurance_sum(cal_range_for_bin_data,:);
