%% MPPCA-ICA code
function [res_PCAICA] = fxn_MPPCA_ICAv02(data_bin_z, data_bin_z_full, prms_forced_IC, prms_ICA_mode, prms_SD_thr,...
...
% % % load('data_mpfc24_mpfc24rawca447950export.mat'); 
% % % data_temp = mpfc24thrint1(1:12000,:); % for debug;
% % % 
% % % bin_frame_num = 40;
...
% % % prms_interest_region2       = [500:550] ;   % green, input second scale. like as [c:d]. disable: [0:0]
...
label_str = {'wise0', 'hand', 'return','wise10', 'pellets', 'return', 'safe', 'pellet', 'return',...
    'wise0', 'hand', 'return', 'safe', 'pellet', 'return', 'wise0', 'hand', 'return',...
    'wise10', 'pellets', 'return', 'safe', 'pellet', 'return', 'wise0', 'hand', 'return',...
    'safe', 'pellet', 'return', 'wise0', 'hand', 'return',  'wise10', 'pellets', 'return'};
          
xticks([44, 60, 64, 80, 92, 177, 193, 203, 236,...
    251, 268, 273, 287, 294, 313, 325, 338, 345,...
    358, 364, 458, 471, 479, 505, 521, 535, 540,...
    554, 565, 586, 599, 611, 616, 629, 635, 735]); 
xticklabels(label_str); xtickangle(90);
hold off
...
figure('Position',[800,0,700,700]); %[left bottom width height] 
...
label_str = {'safe', 'pellet', 'return', 'wise0', 'hand', 'return', 'safe', 'pellet', 'return',...
    'safe', 'pellet', 'return', 'wise10', 'pellets', 'return', 'wise0', 'hand', 'return',...
    'safe', 'pellet', 'return', 'wise0', 'hand', 'return', 'safe', 'pellet', 'return',...
    'wise10', 'pellets', 'return', 'safe', 'pellet', 'return', 'wise0', 'hand', 'return',...
    'postqa', 'NREM', 'preqa', 'NREM', 'TestNS',...
    'wise0', 'hand', 'return', 'wise10', 'pellets', 'return', 'safe', 'pellet', 'return',...
    'wise0', 'hand', 'return', 'safe', 'pellet', 'return', 'wise0', 'hand', 'return',...
    'wise10', 'pellets', 'return', 'safe', 'pellet', 'return', 'wise0', 'hand', 'return',...
    'safe', 'pellet', 'return', 'wise0', 'hand', 'return', 'wise10', 'pellets', 'return',...
    'TestSD', 'wise0', 'hand', 'return', 'safe', 'pellet', 'return', 'wise10', 'pellets', 'return',...
    'safe', 'pellet', 'return', 'wise0', 'hand', 'return', 'safe', 'pellet', 'return',...
    'wise0', 'hand', 'return', 'safe', 'pellet', 'return', 'wise10', 'pellets', 'return',...
    'wise0', 'hand', 'return', 'safe', 'pellet', 'return', 'wise0', 'hand', 'return',...
    'STr', 'safe', 'pellet', 'return', 'unwise0', 'hand', 'return', 'safe', 'pellet', 'return',...
    'safe', 'pellet', 'return', 'unwise2', 'pellets', 'return', 'unwise0', 'hand', 'return',...
    'safe', 'pellet', 'return', 'unwise0', 'hand', 'return', 'safe', 'pellet', 'return',...
    'unwise2', 'pellets', 'return', 'safe', 'pellet', 'return', 'unwise0', 'hand', 'return',...
    'TestS', 'safe', 'pellet', 'return', 'unwise0', 'hand', 'return', 'safe', 'pellet', 'return',...
    'safe', 'pellet', 'return', 'unwise2', 'pellets', 'return', 'unwise0', 'hand', 'return',...
    'safe', 'pellet', 'return', 'safe', 'pellet', 'return', 'unwise0', 'hand', 'return',...
    'safe', 'pellet', 'return', 'unwise2', 'pellets', 'return', 'safe', 'pellet', 'return'};
          
xticks([39, 53, 76, 92, 121, 126, 148, 155, 186,...
    200, 212, 241, 262, 268, 380, 395, 423, 428,...
    447, 458, 494, 509, 526, 533, 552, 562, 581,...
    602, 615, 728, 752, 756, 789, 803, 820, 824,...
    861, 1485, 1661, 2444, 2670,...
    2713,2729,2733,2749,2761,2846,2862,2872,2905,...
    2920,2937,2942,2956,2963,2982,2994,3007,3014,...
    3027,3033,3127,3140,3148,3174,3190,3204,3209,...
    3223,3234,3255,3268,3280,3285,3298,3304,3404,...
    3435, 3479, 3492, 3495, 3511, 3516, 3534, 3550, 3555, 3636,...
    3652, 3658, 3671, 3686, 3703, 3706, 3718, 3723, 3735,...
    3749, 3762, 3766, 3779, 3785, 3805, 3819, 3825, 3904,...
    3918, 3935, 3938, 3950, 3957, 3972, 3985, 3998, 4002,...
    4038, 4054,4059,4087,4104,4121,4125,4140,4149,4171,...
    4186,4191,4208,4223,4231,4258,4272,4290,4294,...
    4310,4319,4332,4347,4368,4372,4389,4394,4414,...
    4433,4439,4457,4474,4482,4502,4516,4533,4536,...
    4572, 4609,4616,4632,4645,4662,4666,4681,4691,4707,...
    4721,4734,4756,4771,4777,4802,4816,4833,4838,...
    4853,4865,4882,4896,4902,4928,4942,4964,4969,...
    4984,4997,5018,5034,5040,5072,5087,5112,5136]); 
xticklabels(label_str); xtickangle(90);
