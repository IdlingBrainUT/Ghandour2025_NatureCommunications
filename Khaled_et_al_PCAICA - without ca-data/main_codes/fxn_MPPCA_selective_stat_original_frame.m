...
function [result_MPPCA_stat] = fxn_MPPCA_selective_stat_original_frame(result_MPPCA, prms_MPPCA)
%% update note
% 211104: 1st code
% 220808: updated by USM request
%% debug mode
...
prms_extra_session_num = prms_MPPCA.prms_extra_session_num  ; %
...
%%
num_of_session = prms_extra_session_num-1;
...
for i = 1:num_of_session
    ...
    result_data_cell_stat{shift+i,7} = find(result_data_cell_stat{shift+i,5} < prms_thr_freq_search );
...
for i = 1:num_of_session   
    result_data_cell_stat{shift+i,10} = result_data_cell_stat{shift+i,9} ./ result_data_cell_stat{shift+prms_stat_ref,9}; % mean   
end

temp_fold_change_stat_RS = {};

for i = 1:num_of_ensemble
temp_fold_change_stat_RS{1,i} = result_data_cell_stat{shift+prms_stat_ref,9}(i) ./ result_data_cell_stat{shift+prms_stat_tar,9}(i);
[temp_fold_change_stat_RS{2,i}, temp_fold_change_stat_RS{3,i}, temp_fold_change_stat_RS{4,i}] = ...
    ranksum(result_data_cell_stat{shift+prms_stat_ref,8}(:,i), result_data_cell_stat{shift+prms_stat_tar,8}(:,i));  
temp_fold_change_stat_RS{5,i} =  temp_fold_change_stat_RS{1,i} >= prms_thr_stat_fold && temp_fold_change_stat_RS{3,i} == 1;
end
%% stat for frequency
for i = 1:num_of_session   
    result_data_cell_stat{shift+i,22} = result_data_cell_stat{shift+i,21} ./ result_data_cell_stat{shift+prms_stat_ref,21}; % mean   
end

temp_fold_change_stat_Frq = {};

for i = 1:num_of_ensemble
temp_fold_change_stat_Frq{1,i} = result_data_cell_stat{shift+prms_stat_ref,21}(i) ./ result_data_cell_stat{shift+prms_stat_tar,21}(i);
[temp_fold_change_stat_Frq{2,i}, temp_fold_change_stat_Frq{3,i}, temp_fold_change_stat_Frq{4,i}] = ...
    ranksum(result_data_cell_stat{shift+prms_stat_ref,19}(:,i), result_data_cell_stat{shift+prms_stat_tar,19}(:,i));  
...
nega_ID(posi_ID) = [];
%% result stat mean
result_stat_mean = cell2mat(result_data_cell_stat(2:end,9)); % 9: raw RS, 10: fold change of RS to target
