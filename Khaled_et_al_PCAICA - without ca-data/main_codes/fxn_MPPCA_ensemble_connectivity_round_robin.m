function result = fxn_MPPCA_ensemble_connectivity_round_robin(result_data_cell, result_MPPCA, prms)
%% comment
% 211118: 1st version
%% decompose prms
...
ensemble_num        = prms.ensemble_num     ;
...
for i = 1:ensemble_num
    for ii = 1:ensemble_num
        [result_connect_cell{i,ii}] = fxn_MPPCA_ensemble_connectivity2 ...
        (result_data_cell, result_MPPCA, session_num_assess, i, ii, ... 
...
for i = 1:ensemble_num
    for ii = 1:ensemble_num
subplot(ensemble_num, ensemble_num, (ensemble_num.*(i-1) + ii))
